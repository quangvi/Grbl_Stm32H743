...
// distance from segment to the circle when the end points both lie on the circle.
    ...
    void mc_arc(double *target, plan_line_data_t *pl_data, double *position, double *offset, double radius,
...
#endif//USER_DOUBLE
{
  ...
  if (segments) {
    ...
    #ifdef USER_MORE_AXIS
      #ifdef USER_DOUBLE
        double linear_per_segment[N_AXIS_2];
      #else
        float linear_per_segment[N_AXIS_2];
      #endif
      uint8_t idx_linear;
...
#ifdef USER_DOUBLE
  uint8_t mc_probe_cycle(double *target, plan_line_data_t *pl_data, uint8_t parser_flags)
#else
...
// NOTE: Uses the always free planner ring buffer head to store motion parameters for execution.
  ...
  #ifdef USER_DOUBLE
    void mc_parking_motion(double *parking_target, plan_line_data_t *pl_data)
  #else
