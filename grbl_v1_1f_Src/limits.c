...
// Homing axis search distance multiplier. Computed by this value times the cycle travel.
  ...
  void limits_init()
  {
    if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
      ...
      #endif
      /*Configure GPIO pins : X_LIMIT_Pin Y_LIMIT_Pin Z_LIMIT_Pin A_LIMIT_Pin */
      #ifdef USER_MORE_AXIS
        #ifdef USER_6_AXIS
          #ifdef USER_8_AXIS
            GPIO_InitStruct.Pin = X_LIMIT_Pin|Y_LIMIT_Pin|Z_LIMIT_Pin|A_LIMIT_Pin|B_LIMIT_Pin|C_LIMIT_Pin|D_LIMIT_Pin|E_LIMIT_Pin;
           #else
            GPIO_InitStruct.Pin = X_LIMIT_Pin|Y_LIMIT_Pin|Z_LIMIT_Pin|A_LIMIT_Pin|B_LIMIT_Pin|C_LIMIT_Pin;
          #endif
        #else
          GPIO_InitStruct.Pin = X_LIMIT_Pin|Y_LIMIT_Pin|Z_LIMIT_Pin|A_LIMIT_Pin;
        #endif
      #else //not USER_MORE_AXIS
//        GPIO_InitStruct.Pin = A_LIMIT_Pin;
...
//        GPIO_InitStruct.Pin = X_LIMIT_Pin|Y_LIMIT_Pin|Z_LIMIT_Pin;
  ...
  void limits_disable()
  {
    ...
    HAL_GPIO_DeInit(LIMIT_PORT_STM32, Z_LIMIT_Pin);
      ...
      #ifdef USER_6_AXIS
        HAL_GPIO_DeInit(LIMIT_PORT_STM32, A_LIMIT_Pin);
        HAL_GPIO_DeInit(LIMIT_PORT_STM32, B_LIMIT_Pin);
        HAL_GPIO_DeInit(LIMIT_PORT_STM32, C_LIMIT_Pin);
        #ifdef USER_8_AXIS
          HAL_GPIO_DeInit(LIMIT_PORT_STM32, D_LIMIT_Pin);
          HAL_GPIO_DeInit(LIMIT_PORT_STM32, E_LIMIT_Pin);
        #endif
      #else
        HAL_GPIO_DeInit(LIMIT_PORT_STM32, A_LIMIT_Pin);
      #endif
...
// number in bit position, i.e. Z_AXIS is (1<<2) or bit 2, and Y_AXIS is (1<<1) or bit 1.
  ...
  uint8_t my_limits_get_state()
  {
    ...
    #ifdef USER_MORE_AXIS
      if (bit_isfalse(LIMIT_PIN_STM32,(A_LIMIT_Pin))) { limit_state |= A_LIMIT_PIN_INDEX; }
      #ifdef USER_6_AXIS
        if (bit_isfalse(LIMIT_PIN_STM32,(B_LIMIT_Pin))) { limit_state |= B_LIMIT_PIN_INDEX; }
        if (bit_isfalse(LIMIT_PIN_STM32,(C_LIMIT_Pin))) { limit_state |= C_LIMIT_PIN_INDEX; }
        #ifdef USER_8_AXIS
          if (bit_isfalse(LIMIT_PIN_STM32,(D_LIMIT_Pin))) { limit_state |= D_LIMIT_PIN_INDEX; }
          if (bit_isfalse(LIMIT_PIN_STM32,(E_LIMIT_Pin))) { limit_state |= E_LIMIT_PIN_INDEX; }
        #endif
      #endif
...
void limits_go_home(uint8_t cycle_mask)
{
  ...
  do {
    ...
    #ifdef USER_BACKLASH
      backlash_line_motion(target, pl_data, true);// homing_rate, false, false, true, HOMING_CYCLE_LINE_NUMBER);
    #else //not USER_BACKLASH
      #ifdef USER_STEP_PWM
        plan_buffer_line_pwm(target, pl_data, true);
      #else
    ...
    do {
      ...
      #else
        } while (STEP_MASK_SIMULATE & axislock);
      #endif
    #else //USER_STM32
