...
#define EXEC_RAPID_OVR_LOW          bit(7)
  ...
  #define EXEC_RAPID_OVR_HIGH       bit(8)
#endif
...
// Define control pin index for Grbl internal use. Pin maps may change, but these values don't.
    ...
    #define CONTROL_PIN_INDEX_SAFETY_DOOR    ((uint8_t)0x01U)//bit(0)
    #define CONTROL_PIN_INDEX_RESET          ((uint8_t)0x02U)//bit(1)
    #define CONTROL_PIN_INDEX_FEED_HOLD      ((uint8_t)0x04U)//bit(2)
    #define CONTROL_PIN_INDEX_CYCLE_START    ((uint8_t)0x08U)//bit(3)
  #else
    #define N_CONTROL_PIN 3
    #define CONTROL_PIN_INDEX_RESET          ((uint8_t)0x01U)//bit(0)
    #define CONTROL_PIN_INDEX_FEED_HOLD      ((uint8_t)0x02U)//bit(1)
    #define CONTROL_PIN_INDEX_CYCLE_START    ((uint8_t)0x04U)//bit(2)
  #endif
#else
  #ifdef ENABLE_SAFETY_DOOR_INPUT_PIN
    #define N_CONTROL_PIN 4
...
extern int32_t sys_probe_position[N_AXIS]; // Last probe position in machine coordinates and steps.
  ...
  extern uint8_t sys_probe_state;   // Probing state value.  Used to coordinate the probing cycle with stepper ISR.
  extern uint8_t sys_rt_exec_state;   // Global realtime executor bitflag variable for state management. See EXEC bitmasks.
  extern uint8_t sys_rt_exec_alarm;   // Global realtime executor bitflag variable for setting various alarms
  #ifdef USER_RAPID_OVR_HIGH
    extern uint16_t sys_rt_exec_motion_override; // Global realtime executor bitflag variable for motion-based overrides.
  #else
    extern uint8_t sys_rt_exec_motion_override; // Global realtime executor bitflag variable for motion-based overrides.
  #endif
  extern uint8_t sys_rt_exec_accessory_override; // Global realtime executor bitflag variable for spindle/coolant overrides.
#else
  ...
  #ifdef USER_RAPID_OVR_HIGH
    extern volatile uint16_t sys_rt_exec_motion_override; // Global realtime executor bitflag variable for motion-based overrides.
  #else
...
void system_flag_wco_change();
  ...
  double system_convert_axis_steps_to_mpos(int32_t *steps, uint8_t idx);
...
#ifdef USER_DOUBLE
  uint8_t system_check_travel_limits(double *target);
#else
...
#ifdef USER_RAPID_OVR_HIGH
  void system_set_exec_motion_override_flag(uint16_t mask);
#else
