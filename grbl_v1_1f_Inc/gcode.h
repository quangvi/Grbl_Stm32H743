...
typedef struct {
    ...
    double f;         // Feed
    #ifdef USER_MORE_AXIS
      double ijk[N_AXIS];    // I,J,K Axis arc offsets
    #else //not USER_A_AXIS_GCODE
      double ijk[3];    // I,J,K Axis arc offsets
    #endif //USER_A_AXIS_GCODE
      uint8_t l;       // G10 or canned cycles parameters
      int32_t n;       // Line number
      double p;         // G10 or dwell parameters
      // double q;      // G82 peck drilling
      double r;         // Arc radius
      double s;         // Spindle speed
      uint8_t t;       // Tool selection
      #ifdef USER_MORE_AXIS
        double xyz[N_AXIS];    // X,Y,Z Translational axes
      #else //not USER_A_AXIS_GCODE
...
typedef struct {
  gc_modal_t modal;
    ...
    double spindle_speed;          // RPM
  #else
    float spindle_speed;          // RPM
  #endif
  #ifdef USER_DOUBLE
    double feed_rate;              // Millimeters/min
    uint8_t tool;                 // Tracks tool number. NOT USED.
    int32_t line_number;          // Last line number sent

    double position[N_AXIS];       // Where the interpreter considers the tool to be at this point in the code

    double coord_system[N_AXIS];    // Current work coordinate system (G54+). Stores offset from absolute machine
                                   // position in mm. Loaded from EEPROM when called.
    double coord_offset[N_AXIS];    // Retains the G92 coordinate offset (work coordinates) relative to
                                   // machine zero in mm. Non-persistent. Cleared upon reset and boot.
    double tool_length_offset;      // Tracks tool length offset value when enabled.
  #else
